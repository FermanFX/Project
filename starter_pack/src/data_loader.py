...
class DataLoader:
    ...
    def load_digits(self) -> Dict[str, np.ndarray]:
        """
        Load digits dataset with fixed train/val/test split.

        Returns:
            Dictionary with keys: X, y, train_idx, val_idx, test_idx
        ...
        # Return raw data and indices (main.py expects this format)
        return {
            'X': X,
            'y': y,
            'train_idx': train_idx,
...
if __name__ == "__main__":
    ...
    print(f"    Full data: X={digits['X'].shape}, y={digits['y'].shape}")
    print(f"    Train indices: {len(digits['train_idx'])}")
    print(f"    Val indices: {len(digits['val_idx'])}")
    print(f"    Test indices: {len(digits['test_idx'])}")
